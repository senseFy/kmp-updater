...
**Maven Central acceptance passed on 2026-09-13.** The [first signed upload](https://github.com/senseFy/kmp-updater/actions/runs/34698188639) passed validation on 2026-09-12; deployment `94eaf3f0-5fb1-41c8-a771-8d9599fdfb36` now reports `PUBLISHED`. All seven publications are publicly downloadable. Independent OpenPGP verification passed for 37 downloaded files, along with metadata references, SHA-256 digests and required publication companions. Evidence: `build/central-downloads/result.json`.

`python3 scripts/verify-release.py --central` passed using a fresh external project that resolves the SDK exclusively from Maven Central. The signed-feed/checkpoint JVM test and all four Native target compilations passed. Evidence: `build/central-verification/result.json`, its consumer log and JUnit results. The publishing workflow's public-availability check also passed; future releases wait for all seven public module files before reporting success.
