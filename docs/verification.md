...
The macOS helper archive contains the production executable with both arm64 and x86_64 slices. GitHub Actions repeats packaging, external consumption and all eight app scenarios before publishing a release. Maven Central's task graph is configured and dry-run checked; uploading awaits namespace verification and publishing credentials.
