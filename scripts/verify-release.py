#!/usr/bin/env python3
"""Consume the Maven ZIP in a fresh project outside the SDK checkout."""
import argparse
import hashlib
import json
...
def main():
    ...
    report_dir = ROOT / "build/release-verification"
        ...
        with (report_dir / "consumer.log").open("w") as log:
            result = subprocess.run(
                [str(consumer / "gradlew"), "verifyPublishedDependencies", "--console=plain",
                 f"-PreleaseRepository={repository.as_uri()}", f"-PupdaterVersion={version}"],
